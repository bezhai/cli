...
func TestCredentialProvider_SelectedExtensionAppSecretMintsTAT(t *testing.T) {
	...
	if result.Source != core.CredentialSourceEnv {
		t.Fatalf("TokenResult.Source = %q, want env", result.Source)
	}
	if source, ok := core.CredentialSourceFromContext(rt.gotReq.Context()); !ok || source != core.CredentialSourceEnv {
		t.Fatalf("TAT request source = %q, want env", source)
	}
