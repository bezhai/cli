...
func TestStaleNotice_Message(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			"lark-cli skills 1.0.20 out of sync with binary 1.0.21, run: lark-cli update --with-skills",
		},
		{
			"official skills unknown",
			StaleNotice{Current: "1.0.21", Target: "1.0.21", OfficialUnknown: true},
			"lark-cli skills were installed from a fallback source; official completeness is unknown, run: lark-cli update --with-skills",
		},
	}
