...
当用户明确要求应用机器人入会承担“会中助手 / 实时关注 / 推送会中聊天或互动 / 总结正在发生的内容”时，入会成功后的默认路径是：

1. 记录 `+meeting-join` 返回的 `meeting.id` 和 `meeting.meeting_no`。
2. 显式启动 `lark-cli event consume vc.bot.meeting_activity_v1 --as bot`，并按 `meeting_no` 过滤本场会议。
3. 同时监听 `vc.bot.meeting_ended_v1`，用它作为停止监听和最终补拉的信号。
4. 监听失败、中断、Agent 重启或需要结构化 `im_post` 时，再用本命令按 `meeting.id` 补拉当前可见事件。

`+meeting-events` 是权威补拉 / 分页 / 结构化转发通道，不是低延迟实时通道。不要让 `+meeting-join` 隐式启动长期后台监听；监听进程必须由 Agent 编排层显式启动、可见、可停止。
...
会中 reaction 在 `+meeting-events --format json` 中表现为：
...
```bash
# 第 1 步：加入会议，记录返回的 meeting.id / meeting.meeting_no
...
# 第 2 步：启动实时监听；按 meeting_no 过滤本场会议
...
# 第 3 步：需要校准、补漏或生成 im_post 时，用 meeting.id 补拉
lark-cli vc +meeting-events --as bot --meeting-id "$MID" --page-all --format pretty
```

收到 `vc.bot.meeting_ended_v1` 后，应停止监听进程，并按需要执行一次 `+meeting-events --page-all --format json` 补拉最终可见事件。
